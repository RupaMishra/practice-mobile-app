...
class BBPSContainer1 extends Component {
  state = {
    contacts: _.flow(
    ...
    pairs: _.flow(
      ...
      (products) =>
        _.map(products, (product) => ({
          ...
          onPress: () =>
            Alert.alert(
              "My price was " + product.formattedPrice + ", now it is $50"
            ),
        }))
    )(products),
    dynamicLayout: _.flow(
      (products) => _.take(products, 3),
      (products) =>
        _.map(products, (product) => ({
          imageProps: {
            source: {
    ...
    return (
      <ScrollView onLayout={this.onLayout}>
        <View padding-page>
          ...
          <GridView
            items={avatars}
            numColumns={Constants.orientation === "landscape" ? 6 : 3}
          />
          <Text marginV-s5 text60BO>
